...
export async function subscribeToStock(stockId: number) {
    try {
        const conn = await startConnection();
        await conn.invoke("SubscribeToStock", stockId);
        console.log("Successfully subscribed to the stock", stockId);
        return true;
    } catch(error) {
...
export async function unsubscribeFromStock(stockId: number) {
    try {
        const conn = await startConnection();
        await conn.invoke("UnsubscribeFromStock", stockId);
        console.log("Successfully unsubscribed from the stock", stockId);
        return true;
    } catch(error) {
        console.error("Error from unsubscription:", error);
        return false;
    }
}
